...
# then log output of file /code/bin/enokid
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/enokid \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/enokid | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.21

COPY --from=build-env /code/build/enokid /usr/bin/enokid
